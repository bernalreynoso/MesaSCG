...
// Extractor de periodo (Año-Mes) para filtrado por periodo
...
export const DashboardSoporte: React.FC<DashboardSoporteProps> = ({
  tickets,
  onSelectTicket,
  onLoadDemoData,
}) => {
  const { tecnicosSoporteActivos, tecnicosActivos } = useTecnicosCatalogo();

  // Lista de técnicos activos disponibles
  const tecnicosDisponibles = useMemo(() => {
    if (tecnicosSoporteActivos.length > 0) {
      return tecnicosSoporteActivos;
    }
    return tecnicosActivos;
  }, [tecnicosSoporteActivos, tecnicosActivos]);

  const [selectedTechName, setSelectedTechName] = useState<string>(() => {
    return tecnicosDisponibles[0]?.nombre || 'Francisco Bernal Reynoso';
  });
  ...
  const techInfo = useMemo(() => {
    ...
    return (
      tecnicosDisponibles[0] || {
        id: 'default',
        nombre: selectedTechName || 'Francisco Bernal Reynoso',
  ...
  const [lineaSel, setLineaSel] = useState<string>('TODOS');
  ...
  useEffect(() => {
    setPaginaActual(1);
  }, [selectedTechName, participacionSel, estadoSel, tipoTrabajoSel, periodoSel, lineaSel, catSel, subSel, artSel]);
  ...
  // Tickets base del técnico (con cualquier interacción o evidencia, o donde es responsable)
  ...
  // 2. Cálculo de métricas KPI canónicas para el técnico (NO mezclar CC con atendidos)
  const metricasKPI = useMemo(() => {
    ...
    // Solo calculamos sobre los tickets donde es responsable o tiene evidencia fuerte para atendidos
  ...
  const ticketsFiltrados = useMemo(() => {
    return ticketsEvaluadosTecnico.filter((item) => {
      ...
      // 3. Filtro por Tipo de Trabajo (Motor Contextual)
      ...
      // 5. Filtro por Línea de Trabajo (si aplica)
      if (techInfo.filtroDesarrollo && lineaSel !== 'TODOS') {
        const tipo = String(item.ticket.tipoSolicitud || '').toUpperCase();
        const cat = String(item.ticket.categoria || '').toUpperCase();
        const esDesarrollo = tipo.includes('SOPORTE') && cat.includes('SISTEMAS');
        if (lineaSel === 'Desarrollo' && !esDesarrollo) return false;
        if (lineaSel === 'Soporte' && esDesarrollo) return false;
      }

      // 6. Filtros de Catálogo (Categoría, Subcategoría, Artículo)
      if (catSel !== 'TODAS' && item.ticket.categoria !== catSel) return false;
      if (subSel !== 'TODAS' && item.ticket.subcategoria !== subSel) return false;
  ...
  const totalPaginas = Math.max(1, Math.ceil(ticketsFiltrados.length / ELEMENTOS_POR_PAGINA));
  const ticketsPagina = useMemo(() => {
    const inicio = (paginaActual - 1) * ELEMENTOS_POR_PAGINA;
    return ticketsFiltrados.slice(inicio, inicio + ELEMENTOS_POR_PAGINA);
  }, [ticketsFiltrados, paginaActual]);

  // Listas desplegables reactivas
  ...
  // Datos para Gráfica de Artículo por Estado
  ...
  const COLORS = ['#4f46e5', '#0284c7', '#7c3aed', '#d97706', '#db2777', '#059669'];
  ...
  if (tickets.length === 0) {
    return (
      <EmptyState
        title="No hay tickets en el Control de Soporte Técnico"
        description="Por favor, carga un reporte Excel de ServiceDesk Plus para analizar el rendimiento y carga de los técnicos de soporte."
        ...
        <div className="flex items-start gap-3.5">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-900">
              Control de Carga por Técnico de Soporte
            </h2>
      ...
      <div className="bg-white border border-slate-200/80 p-6 rounded-2xl shadow-xs space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-200">
          <div className="w-full md:w-80">
            <label className="text-xs font-bold text-slate-700 block mb-1.5">
              1. Selecciona el Técnico de Soporte:
            </label>
        ...
        <div className="bg-slate-50/90 border border-slate-200/90 p-4 rounded-xl space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            ...
            {/* Subcategoría */}
            <div className="space-y-1">
              <label className="text-[11px] font-bold text-slate-700">Subcategoría:</label>
              <select
                value={subSel}
                onChange={(e) => {
            ...
            {/* Línea de trabajo si aplica */}
            {techInfo.filtroDesarrollo && (
              <div className="space-y-1">
                <label className="text-[11px] font-bold text-slate-700">Línea de Trabajo:</label>
                <select
                  value={lineaSel}
        ...
        {totalPaginas > 1 && (
          <div className="pt-3 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
            <span className="text-slate-500 font-medium">
              Página <strong className="text-slate-900">{paginaActual}</strong> de <strong className="text-slate-900">{totalPaginas}</strong> ({ticketsFiltrados.length} tickets en total)
            </span>
